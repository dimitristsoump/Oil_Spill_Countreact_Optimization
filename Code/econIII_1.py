...
def compute_fitness(generation):
    ...
    for unit in generation:
        metric1,metric2,boom_length,result_rem=strategy(unit)
    ...
    return fit_table,length_table,rem_to_draw

def add_reuse(int_supp,row_reu,row_loa,n_reu,n_loa,demm):#genika prepei len(row_reu)==n_reu
    while len(row_loa)>n_loa: #prosthetoume osa skimmers mporoun na ksanaxrhsimopoihthoun
        a=row_loa.pop()
        int_supp+=a
    for k in range(len(row_reu)-1,-1,-1): #psaxnoume apo pisw pros ta mporws gia to prwto meriko athroisma pou tha vgalei 
                                          #thn demmanding posothta 
        if sum(row_reu[k:len(row_reu)])>=demm and k==len(row_reu)-1:
            row_reu[k]-=demm
        ...
        if sum(row_reu[k:len(row_reu)])>=demm and k<len(row_reu)-1:
            ...
            #print(k,"hdhn")
            a=demm-sum(row_reu[k:len(row_reu)])
            if a<=int_supp :
                int_supp-=a
                row_reu.insert(0,a)
                b=row_reu.pop()
...
def strategy(unit):
    ...
    *disp_control_metric= mean(std_deviations between the new disperse curve and the standard one for each hour)
    ...
    distance_from_closest_coast=250 #kmr
    tug_speed=25.96 #km/hour
    time_for_reuse=19.26
    time_step=3 # the time steps we choose
    num_of_time_steps=int(time_for_reuse/time_tep)+1
    ...
    row_skimm_s=[] # number of skimmers_s that will be available for reuse
    row_reuse_s=[] # number of skimmers that can still be reused 
    row_skimm_l=[] # number of skimmers_l that will be available for reuse
    row_reuse_l=[] # number of skimmers that can still be reused
    ...
    for goal in rem_req:
        ...
        remain_small=0
        remain_large=0
        num_large=int(0.7*goal/(skimm_l.vi*0.863*3))
        num_small=int(0.3*goal/(skimm_s.vi*0.863*3))
        ...
        if (skimm_l.num==0): # trying to cover the skimm_s demand by skimm_l supply
            ...
            if (remain_small<=skimm_l.vi*0.863*3*skimm_l.num):
                num_large=int(remain_small/(skimm_l.vi*0.863*3))
                skimm_l.use(num_large)
                large+=num_large
            else:
        ...
        if (skimm_s.num==0): # trying to cover the skimm_l demand by skimm_s supply
            ...
            if (remain_large<=skimm_s.vi*0.863*3*skimm_s.num):
                num_small=int(remain_large/(skimm_s.vi*0.863*3))
                skimm_s.use(num_small)
                small+=num_small
            else:
    ...
    return rem_control_metric, disp_control_metric,perimeter,result_rem
...
def main():
    ...
    for gen in range(0,maxgens):
        ...
        fitness,boom_length,rem=compute_fitness(ind)
        min_fit=min(fitness)
        #print(fitness)
    ...
    print ("Number of small skimmers needed: "+ str(int(sum(fittest_ind[0].used))))
    print ("Number of large skimmers needed: "+ str(int(sum(fittest_ind[1].used))))

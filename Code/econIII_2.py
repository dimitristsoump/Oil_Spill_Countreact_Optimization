...
def create_individuals(howmany,fromold):
    ...
    for ii in range(0,d):
        skim_small=skimmers(15,100,np.random.randint(1,50)) #https://www.globalspill.com.au/product/weir-skimmer-30000-lhr-gsw30esp/
        skim_large=skimmers(30,100,np.random.randint(1,50))
...
def strategy(unit):
    ...
    *disp_control_metric= mean(std_deviations between the new disperse curve and the standard one for each hour)
    ...
    time_for_reuse=6#2*distance_from_closest_coast/(tug_speed*3600/1000)
    ...
    for goal in rem_req:
        ...
        skimm_s.used.append(skimm_s.tnum-num_small_rem)
        skimm_l.used.append(skimm_l.tnum-num_large_rem)
        supplies_s.append(skimm_s.num)
        supplies_l.append(skimm_l.num)
